...
const TEMPLATES = {
  en: {
    ...
    keyLines: (v) => [
      `Hi ${v.name},`,
      `We checked your listing fee for “${v.title}” (reference ${v.ref}) and approved it. To publish the listing, enter this key on Vòng:`,
      '{{KEY}}',
      `Enter it here: ${v.url}`,
      'The key works for 7 days. If you didn’t post this listing, ignore this email.',
      'The Vòng team',
    ],
    rejectSubject: (v) => `About your Vòng listing: ${v.title}`,
    rejectLines: (v) => [
      `Hi ${v.name},`,
      `We couldn’t publish your listing “${v.title}” yet.`,
      `Reason: ${v.reason}`,
      `Your reference is ${v.ref}. If you’ve paid since, or think this is a mistake, open your listing and press “I’ve sent the payment” again: ${v.url}`,
      'The Vòng team',
    ],
  },
  vi: {
    ...
    keyLines: (v) => [
      `Chào ${v.name},`,
      `Tụi mình đã kiểm tra phí đăng tin cho “${v.title}” (mã tin ${v.ref}) và duyệt tin của bạn. Để đăng tin, bạn nhập mã này trên Vòng:`,
      '{{KEY}}',
      `Nhập mã tại: ${v.url}`,
      'Mã dùng được trong 7 ngày. Nếu bạn không đăng tin này thì cứ bỏ qua email này.',
      'Nhóm Vòng',
    ],
    rejectSubject: (v) => `Về tin đăng Vòng của bạn: ${v.title}`,
    rejectLines: (v) => [
      `Chào ${v.name},`,
      `Tụi mình chưa đăng được tin “${v.title}” của bạn.`,
      `Lý do: ${v.reason}`,
      `Mã tin của bạn là ${v.ref}. Nếu bạn đã chuyển khoản sau đó, hoặc thấy có nhầm lẫn, bạn mở tin và bấm “Mình đã chuyển khoản” lần nữa: ${v.url}`,
      'Nhóm Vòng',
    ],
  },
};
