...
import { issuePublishKey, sendRejection, siteUrl } from '../publish-key.js';
...
const ADMIN_COLUMNS = `
  ...
  fee_vnd, views, created_at, paid_marked_at, reviewed_at, published_at, publish_key_sent_at
`;
const STATUSES = ['pending_payment', 'awaiting_approval', 'approved', 'published', 'rejected'];
...
/**
 * POST /api/admin/listings/:id/approve — you saw the money arrive. The listing
 * is not live yet: a one-time key goes to the seller's email, and it publishes
 * when they enter it. If the email could not be sent, the key comes back here
 * so you can pass it on yourself.
 */
router.post('/listings/:id/approve', async (req, res, next) => {
  try {
    ...
    if (!row.seller_email) return res.status(409).json({ error: 'no_seller_email' });

    db.prepare("UPDATE listings SET status = 'approved', reviewed_at = ?, reject_reason = NULL WHERE id = ?")
      .run(new Date().toISOString(), row.id);
    const delivery = await issuePublishKey(row, siteUrl(req));
    res.json({ id: row.id, status: 'approved', ...delivery });
  } catch (error) { next(error); }
});

/** POST /api/admin/listings/:id/resend-key — replaces the key and sends it again. */
router.post('/listings/:id/resend-key', async (req, res, next) => {
  try {
    const row = db.prepare('SELECT * FROM listings WHERE id = ?').get(req.params.id);
    if (!row) return res.status(404).json({ error: 'not_found' });
...
router.post('/listings/:id/reject', async (req, res, next) => {
  try {
    ...
    db.prepare(`UPDATE listings SET status = 'rejected', reject_reason = ?, reviewed_at = ?,
      publish_key_hash = NULL, publish_key_expires_at = NULL WHERE id = ?`)
      .run(reason, new Date().toISOString(), row.id);
    const { delivery } = await sendRejection(row, reason, siteUrl(req));
    res.json({ id: row.id, status: 'rejected', reject_reason: reason, delivery });
  } catch (error) { next(error); }
});

/** GET/PUT /api/admin/settings — bank details and the listing fee. */
router.get('/settings', (req, res) => {
  res.json({ settings: getSettings(), banks: BANKS, default_password: usingDefaultPassword(), mail_configured: mailConfigured() });
});
